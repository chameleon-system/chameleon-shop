...
<div class="TPkgImageHotspotItem">
    <div class="standard">
        <img src="<?php echo TGlobal::OutHTML($oImage->GetFullURL()); ?>" alt="<?php echo TGlobal::OutHTML($oItem->fieldName); ?>"
             width="<?php echo TGlobal::OutHTML($oImage->aData['width']); ?>"
             height="<?php echo TGlobal::OutHTML($oImage->aData['height']); ?>" border="0" usemap="#map<?php echo $oItem->id; ?>"/>
        <?php
        $aArea = [];
...
$oSpots->GoToStart();
?>
        <map class="image-map" name="map<?php echo $oItem->id; ?>">
            <?php
    while ($oSpot = $oSpots->Next()) {
