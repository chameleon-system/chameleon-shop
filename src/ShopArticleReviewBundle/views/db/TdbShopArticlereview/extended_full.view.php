...
<div class="reviewitem">
    ...
    <div class="reportreview"><a
        href="<?=$oReview->GetReportURL(); ?>"><?=TGlobal::OutHTML(\ChameleonSystem\CoreBundle\ServiceLocator::get('translator')->trans('chameleon_system_shop_article_review.action.report_as_inappropriate')); ?></a>
    </div>
    <?php
} ?>
    <div class="ownreview"><a href=""><?=TGlobal::OutHTML(\ChameleonSystem\CoreBundle\ServiceLocator::get('translator')->trans('chameleon_system_shop_article_review.action.write_review')); ?></a></div>
    <?php if ($bAllowRateReviews) {
        ?>
    <?php if ($oReview->fieldHelpfulCount > 0) {
            ?>
        <div class="rateoverview">
            <?=TGlobal::OutHtml(\ChameleonSystem\CoreBundle\ServiceLocator::get('translator')->trans('chameleon_system_shop_article_review.text.how_helpful',
                array(
    ...
    <div class="ratereview">
        <div class="pro_rate"><a
            href="<?=$oReview->GetRateURL(true); ?>"><?=TGlobal::OutHTML(\ChameleonSystem\CoreBundle\ServiceLocator::get('translator')->trans('chameleon_system_shop_article_review.action.vote_up')); ?></a>
        </div>
        <div class="contra_rate"><a
            href="<?=$oReview->GetRateURL(false); ?>"><?=TGlobal::OutHTML(\ChameleonSystem\CoreBundle\ServiceLocator::get('translator')->trans('chameleon_system_shop_article_review.action.vote_down')); ?></a>
        </div>
    </div>
    <?php
    }?>
    ...
    if ((!$oPkgCommentModuleConfig->fieldGuestCommentAllowed && $oUser->IsLoggedIn()) || $oPkgCommentModuleConfig->fieldGuestCommentAllowed) {
        ?>
        <a href=""
           onclick="$('.reviecommentform-<?=TGlobal::OutHTML($oReview->id); ?>').show();return false"><?=TGlobal::OutHTML(\ChameleonSystem\CoreBundle\ServiceLocator::get('translator')->trans('chameleon_system_shop_article_review.action.comment_on_review')); ?></a>
        <div class="reviecommentform-<?=TGlobal::OutHTML($oReview->id); ?> hide">
            <div class="writecommentforreview"><?=TGlobal::OutHTML(\ChameleonSystem\CoreBundle\ServiceLocator::get('translator')->trans('chameleon_system_shop_article_review.text.comment_on_review_headline')); ?></div>
            <form name="reguser" accept-charset="utf-8" method="post" action="" enctype="multipart/form-data">
                ...
                <input type="submit" name="savecomment" class="button_savecomment"
                       value="<?=TGlobal::OutHTML(\ChameleonSystem\CoreBundle\ServiceLocator::get('translator')->trans('chameleon_system_shop_article_review.action.submit_comment')); ?>"/>
            </form>
        </div>
        <?php
    } else {
