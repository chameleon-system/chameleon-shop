                    ...
                    <div class="jscomment_inner" style="<?php if (!$oComment->IsActiveComment()) {
                        ...
                        <form name="recomment-<?=TGlobal::OutHTML($oComment->id); ?>" accept-charset="utf-8" method="post"
                            ...
                            <input type="submit" name="savecomment" id="editbuton-<?=TGlobal::OutHTML($oComment->id); ?>"
                                   ...
                                   value="<?=TGlobal::OutHTML(TGlobal::Translate('chameleon_system_shop_article_review.action.refresh')); ?>"/>
                        </form>
                    </div>
                    ...
                    <?php if ($oComment->fieldMarkAsReported) {
                        ...
                        <?php if ($sReCommentURL) {
                            ...
                            <div class="recomment <?=$sCmsIdent; ?>"><a href="<?=$sReCommentURL; ?>"
                                                                      onclick="RespondComment('<?=TGlobal::OutHTML($oComment->id); ?>','<?=TGlobal::OutHTML($sCmsIdent); ?>');$(this).parent().siblings('.jscomment_inner').show(400);return false;"><?=TGlobal::OutHTML(TGlobal::Translate('chameleon_system_shop_article_review.action.respond_to_comment')); ?></a>
                            </div>
                            <?php
                                } ?>
                        <?php
