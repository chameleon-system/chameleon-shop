...
class SimpleCheckoutResult extends WebServerExample
{
    ...
    public function run()
    {
        ...
        $amazonAuthorizationId = $this->_authorizeOrderAmount($orderTotal);
    ...
    private function _addOrderTotalAndSellerInformationToOrder($orderTotal)
    {
        $response
            = $this->callStepAndCheckForException(
                ...
                array($orderTotal, $this->currencyCode)
            );
     ...
     *                id reference
     */
    private function _authorizeOrderAmount($authorizationAmount)
    {
        $response = $this->callStepAndCheckForException(
            'authorizeOrderAmount',
            array($authorizationAmount, $this->currencyCode)
        );
    ...
    private function _captureOrderAmount($captureAmount, $amazonAuthorizationId)
    {
        $response = $this->callStepAndCheckForException(
            'captureOrderAmount',
            array($captureAmount, $amazonAuthorizationId, $this->currencyCode)
        );
