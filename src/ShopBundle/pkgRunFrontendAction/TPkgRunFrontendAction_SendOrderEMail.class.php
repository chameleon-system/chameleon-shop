    ...
    public function runAction($aParameter)
    {
        ...
        if (isset($aParameter['email']) && isset($aParameter['order_id']) && !empty($aParameter['email']) && !empty($aParameter['order_id'])) {
            ...
            if ($oOrder->Load($aParameter['order_id'])) {
                ...
                if ($bSuccess) {
                    $oStatus->sMessage = ChameleonSystem\CoreBundle\ServiceLocator::get('translator')->trans('chameleon_system_shop.orders.msg_order_confirm_sent', ['%mail%' => $aParameter['email']]);
                    $oStatus->sMessageType = 'MESSAGE';
                } else {
                    $oStatus->sMessage = ChameleonSystem\CoreBundle\ServiceLocator::get('translator')->trans('chameleon_system_shop.orders.error_sending_confirm_mail', ['%error%' => $bSuccess]);
