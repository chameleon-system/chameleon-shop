...
class TShopModuleArticleListFilterOfferArticles extends TdbShopModuleArticleListFilter
{
    /**
    ...
    protected function GetListQueryBase($oListConfig)
    {
        $sQuery = "SELECT 0 AS cms_search_weight, `shop_article`.*
                      ...
                      LEFT JOIN `shop_article_stock` ON `shop_article`.`id` = `shop_article_stock`.`shop_article_id`
                          WHERE `shop_article`.`price_reference`  NOT LIKE '0.00'
                            AND `shop_article`.`price_reference` > `shop_article`.`price`
                        ";
