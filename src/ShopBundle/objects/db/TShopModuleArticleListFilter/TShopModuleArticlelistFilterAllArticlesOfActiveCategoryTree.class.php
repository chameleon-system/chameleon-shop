        ...
        if (null !== $activeCategory) {
            ...
            $query = "SELECT DISTINCT 0 AS cms_search_weight, `shop_article`.*
                ...
                LEFT JOIN `shop_article_stock` ON `shop_article`.`id` = `shop_article_stock`.`shop_article_id`
                    WHERE `shop_article`.`id` IN (SELECT DISTINCT `shop_article_shop_category_mlt`.`source_id` FROM `shop_article_shop_category_mlt` WHERE `shop_article_shop_category_mlt`.`target_id` IN ('".implode("', '", $categories)."'))
                ";
        } else {
