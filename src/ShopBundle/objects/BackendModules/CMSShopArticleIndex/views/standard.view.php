...
<div class="card">
    <div class="card-header">
        <?= $translator->trans('chameleon_system_shop.product_search_index.headline'); ?>
    </div>
    <div class="card-body">
        <form name="indexer" method="post" action="<?=PATH_CMS_CONTROLLER; ?>" accept-charset="UTF-8">
            ...
            <?php if (!$data['bIndexIsRunning'] || $data['bIndexCompleted']) {
                ?>
                <input class="btn btn-primary" type="submit" value="<?= $translator->trans('chameleon_system_shop.product_search_index.generate'); ?>"/>
                <?php
            } ?>
        </form>
        <?php
        ...
        ?>
        <div class="mt-4"><?= $translator->trans('chameleon_system_shop.product_search_index.completed'); ?>: <?=$oLocal->FormatNumber($data['bPercentDone'], 2); ?>%</div>
